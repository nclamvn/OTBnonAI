...
═══════════════════════════════════════════════ */
const StatusPipeline = ({ stats, darkMode, statusFilter, setStatusFilter }: any) => {
  const { t } = useLanguage();
  const steps = [
        ...
        return (
          <button
            ...
            className={`flex-1 relative rounded-xl border px-3 py-2.5 transition-all duration-200 group ${
              isActive
                ? darkMode
                  ? 'border-[#D7B797]/40 bg-[rgba(215,183,151,0.08)]'
                  : 'border-[#D7B797]/60 bg-[rgba(215,183,151,0.08)]'
                : darkMode
                  ? 'border-[#2E2E2E] bg-[#121212] hover:border-[#3E3E3E] hover:bg-[#1A1A1A]'
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div className="text-left min-w-0">
                <p className={`text-[10px] font-medium uppercase tracking-wider font-['Montserrat'] ${darkMode ? 'text-[#666666]' : 'text-gray-500'}`}>
                  {step.label}
                </p>
                <p className={`text-base font-bold font-['JetBrains_Mono'] leading-tight ${darkMode ? 'text-[#F2F2F2]' : 'text-gray-900'}`}>
                  {step.count}
                </p>
              </div>
            </div>
    ...
    <div className={`px-4 py-3 border-t ${border} ${darkMode ? 'bg-[#0D0D0D]' : 'bg-[#FAFAF8]'}`}>
      <div className="space-y-1.5">
        {products.map((item: any, idx: number) => {
          ...
          return (
            <div key={item.sku || idx} className={`rounded-xl border overflow-hidden transition-all ${darkMode ? 'border-[#2E2E2E] bg-[#121212]' : 'border-gray-200 bg-white'} ${isExpanded ? 'shadow-sm' : ''}`}>
              {/* SKU Header */}
              <div
                className={`flex items-center gap-3 px-3.5 py-2.5 cursor-pointer transition-colors ${darkMode ? 'hover:bg-[#1A1A1A]' : 'hover:bg-gray-50'}`}
              ...
              >
                ...
                <div className={`w-9 h-9 rounded-lg border flex items-center justify-center shrink-0 ${darkMode ? 'bg-[#1A1A1A] border-[#2E2E2E]' : 'bg-gray-50 border-gray-200'}`}>
                  <ImageIcon size={14} className={textMuted} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className={`text-xs font-semibold font-['Montserrat'] truncate ${textPrimary}`}>
                    <span className={`font-['JetBrains_Mono'] ${darkMode ? 'text-[#D7B797]' : 'text-[#8B6914]'}`}>{item.sku}</span>
                ...
                <div className="flex items-center gap-5 shrink-0">
                  ...
                  <div className="text-right">
                    ...
                    <div className={`text-xs font-bold font-['JetBrains_Mono'] ${darkMode ? 'text-[#D7B797]' : 'text-[#8B6914]'}`}>{formatCurrency(ttlValue)}</div>
                  </div>
                </div>
              </div>
                ...
                <div className={`px-3.5 pb-3 space-y-2.5 border-t ${border}`}>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2.5">
                    {[
                      ...
                      <div key={d.label} className={`rounded-lg border px-3 py-2 ${darkMode ? 'bg-[#1A1A1A]/60 border-[#2E2E2E]' : 'bg-gray-50 border-gray-200'}`}>
                        <p className={`text-[10px] uppercase tracking-wide ${textMuted}`}>{d.label}</p>
                        <p className={`text-xs font-semibold mt-0.5 ${d.mono ? "font-['JetBrains_Mono']" : ''} ${d.accent ? (darkMode ? 'text-[#D7B797]' : 'text-[#8B6914]') : textPrimary}`}>{d.value}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  {(() => {
                    ...
                    return (
                      <div className={`rounded-xl border overflow-hidden ${darkMode ? 'border-[#2E2E2E]' : 'border-gray-200'}`}>
                        <div className={`px-3.5 py-1.5 text-xs font-semibold border-b font-['Montserrat'] ${darkMode ? 'text-[#F2F2F2] bg-[#1A1A1A] border-[#2E2E2E]' : 'text-gray-600 bg-gray-50 border-gray-200'}`}>
                          Store × Size Allocation
                        </div>
                        <div className="overflow-x-auto">
                          <table className="w-full text-xs">
                            <thead>
                              <tr className={darkMode ? 'bg-[#1A1A1A] text-[#999999]' : 'bg-[rgba(160,120,75,0.08)] text-[#666666]'}>
                            ...
                            <tbody>
                              {storeRows.map((store) => (
                                <tr key={store.key} className={`border-t ${darkMode ? 'border-[#2E2E2E]' : 'border-gray-100'}`}>
                              ...
                              <tr className={`border-t-2 ${darkMode ? 'border-[#D7B797]/20 bg-[rgba(215,183,151,0.04)]' : 'border-[#D7B797]/30 bg-[rgba(160,120,75,0.04)]'}`}>
                                <td className={`px-3.5 py-1.5 font-semibold ${darkMode ? 'text-[#D7B797]' : 'text-[#8B6914]'}`}>Total</td>
                                {sizeKeys.length > 0
                                  ? sizeKeys.map(s => {
                      ...
                      <div className={`rounded-xl border overflow-hidden ${darkMode ? 'border-[#2E2E2E]' : 'border-gray-200'}`}>
                        ...
                        <div className="overflow-x-auto">
                          <table className="w-full text-xs">
                            ...
                            <tbody>
                              <tr className={`border-t ${darkMode ? 'border-[#2E2E2E]' : 'border-gray-100'}`}>
                                <td className={`px-3.5 py-1.5 ${darkMode ? 'text-[#F2F2F2]' : 'text-gray-700'}`}>% Sales mix</td>
                                {sizeKeys.map(s => (
                              ...
                              <tr className={`border-t ${darkMode ? 'border-[#2E2E2E]' : 'border-gray-100'}`}>
                                <td className={`px-3.5 py-1.5 ${darkMode ? 'text-[#F2F2F2]' : 'text-gray-700'}`}>% ST</td>
                                {sizeKeys.map(s => (
                                  <td key={s} className={`px-3 py-1.5 text-center font-['JetBrains_Mono'] ${darkMode ? 'text-[#999999]' : 'text-gray-600'}`}>{sizes[s].st}%</td>
                                ))}
                                <td className={`px-3 py-1.5 text-center ${darkMode ? 'text-[#666666]' : 'text-gray-500'}`}>-</td>
                              </tr>
                              <tr className={`border-t ${darkMode ? 'border-[#2E2E2E] bg-[rgba(215,183,151,0.06)]' : 'border-gray-200 bg-[rgba(160,120,75,0.06)]'}`}>
                                <td className={`px-3.5 py-1.5 font-semibold ${darkMode ? 'text-[#D7B797]' : 'text-[#8B6914]'}`}>Qty</td>
                                {sizeKeys.map(s => {
                                  const qty = Math.round(totalQty * sizes[s].salesMix / 100);
                                  return <td key={s} className={`px-3 py-1.5 text-center font-['JetBrains_Mono'] font-semibold ${darkMode ? 'text-[#F2F2F2]' : 'text-gray-800'}`}>{qty}</td>;
                                })}
                                <td className={`px-3 py-1.5 text-center font-bold font-['JetBrains_Mono'] ${darkMode ? 'text-[#F2F2F2]' : 'text-gray-800'}`}>{totalQty}</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
...
const OrderConfirmationScreen = ({ darkMode }: any) => {
  ...
  return (
    <div className={`min-h-full ${darkMode ? 'bg-[#0A0A0A]' : 'bg-[#F8F7F4]'}`}>
      ...
      >
        ...
        <div className="flex items-center justify-between gap-3 mb-2.5">
          <div className="flex items-center gap-3 md:gap-5">
            ...
            <div>
              ...
              <p className={`text-lg md:text-xl font-bold font-['JetBrains_Mono'] leading-tight ${darkMode ? 'text-[#D7B797]' : 'text-[#8B6914]'}`}>
        ...
        {/* Status Pipeline */}
        <StatusPipeline stats={stats} darkMode={darkMode} statusFilter={statusFilter} setStatusFilter={setStatusFilter} />
      </div>
      ...
      <div className={`border ${border} rounded-2xl overflow-hidden mt-3`} style={{
          ...
          <div className={`px-4 py-2.5 flex items-center justify-between border-b ${border}`}>
            <div className="flex items-center gap-2">
              <BarChart3 size={14} className={darkMode ? 'text-[#D7B797]' : 'text-[#8B6914]'} />
              ...
              <span className={`text-[10px] font-['JetBrains_Mono'] px-1.5 py-0.5 rounded ${darkMode ? 'bg-[#1A1A1A] text-[#999999]' : 'bg-gray-100 text-gray-600'}`}>
                {filtered.length}
              </span>
            </div>
            {statusFilter !== 'all' && (
              <button
                ...
                className={`text-[10px] font-medium font-['Montserrat'] px-2 py-1 rounded-md transition-colors ${darkMode ? 'text-[#999999] hover:bg-[#1A1A1A]' : 'text-gray-500 hover:bg-gray-100'}`}
              >
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20">
              <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-4 ${darkMode ? 'bg-[#1A1A1A]' : 'bg-gray-100'}`}>
                  ...
                  <div key={order.id || idx}>
                    <div
                      className={`rounded-xl border overflow-hidden transition-all ${border} ${
                        isExpanded
                          ? darkMode ? 'bg-[rgba(215,183,151,0.04)]' : 'bg-[rgba(215,183,151,0.04)]'
                      ...
                      >
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-2">
                          ...
                          <div className="text-right">
                            ...
                            <p className={`text-xs font-bold font-['JetBrains_Mono'] ${darkMode ? 'text-[#D7B797]' : 'text-[#8B6914]'}`}>{formatCurrency(order.totalValue)}</p>
                          </div>
                        </div>
                    ...
                    {isExpanded && <OrderDetailPanel order={order} darkMode={darkMode} />}
                  </div>
                );
              })}
            </div>
                      ...
                      <Fragment key={order.id || idx}>
                        <tr
                          className={`border-b ${border} transition-colors cursor-pointer ${
                            isExpanded
                              ? darkMode ? 'bg-[rgba(215,183,151,0.04)]' : 'bg-[rgba(215,183,151,0.04)]'
                              : darkMode ? 'hover:bg-[#161616]' : 'hover:bg-[#FAFAF8]'
                          }`}
                          onClick={() => setExpandedOrderId(isExpanded ? null : order.id)}
                        >
                          ...
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center justify-center w-7 h-7 rounded-lg text-xs font-bold font-['JetBrains_Mono'] ${darkMode ? 'bg-[#1A1A1A] text-[#F2F2F2]' : 'bg-gray-100 text-gray-800'}`}>
                              {order.skuCount || order.products?.length || 0}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <span className={`text-sm font-bold font-['JetBrains_Mono'] ${darkMode ? 'text-[#D7B797]' : 'text-[#8B6914]'}`}>
      ...
      {confirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setConfirmModal(null)}>
          <div
            className={`w-full max-w-md mx-4 rounded-2xl border ${border} shadow-2xl overflow-hidden ${darkMode ? 'bg-[#141414]' : 'bg-white'}`}
          ...
          >
            ...
            <div className="p-5">
              <div className={`rounded-xl border ${border} overflow-hidden`}>
                {[
                  ...
                  <div key={row.label} className={`flex items-center justify-between px-4 py-2.5 ${i > 0 ? `border-t ${border}` : ''} ${darkMode ? 'bg-[#1A1A1A]' : 'bg-gray-50'}`}>
                    <span className={`text-xs ${textMuted}`}>{row.label}</span>
                    <span className={`text-sm font-semibold ${row.mono ? "font-['JetBrains_Mono']" : "font-['Montserrat']"} ${row.accent ? (darkMode ? 'text-[#D7B797]' : 'text-[#8B6914]') : textPrimary}`}>
                      {row.value}
                    </span>
                  </div>
                ))}
              ...
              {confirmModal.action === 'cancel' && (
                <div className={`flex items-start gap-2 mt-4 p-3 rounded-xl ${darkMode ? 'bg-[rgba(248,81,73,0.06)]' : 'bg-red-50'}`}>
                  <AlertTriangle size={14} className="text-[#F85149] mt-0.5 shrink-0" />
                  <p className={`text-xs ${darkMode ? 'text-[#FF7B72]' : 'text-red-600'}`}>
            ...
            <div className={`px-5 py-4 border-t ${border} flex justify-end gap-3`}>
              <button
                ...
                className={`px-4 py-2 rounded-xl border ${border} text-xs font-medium font-['Montserrat'] ${textSecondary} transition-all ${darkMode ? 'hover:bg-[#1A1A1A]' : 'hover:bg-gray-100'}`}
              >
                {t('common.back')}
              </button>
              <button
