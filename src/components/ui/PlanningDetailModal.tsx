...
const PlanningDetailModal = ({
  ...
  const filterOptions = useMemo(() => {
    const genders: any[] = [{ id: 'all', name: t('planningDetail.allGenders') }];
    const categories: any[] = [{ id: 'all', name: t('planningDetail.allCategories') }];
    const subCategories: any[] = [{ id: 'all', name: t('planningDetail.allSubCategories') }];
  ...
  const filteredSubCategoryOptions = useMemo(() => {
    ...
    return [{ id: 'all', name: t('planningDetail.allSubCategories') }, ...options.filter((o: any) => o.id !== 'all')];
  ...
  const renderCollectionTab = () => {
    const collectionData = COLLECTION_SECTIONS.map((section: any, sectionIdx: any) => {
      ...
      return {
        ...
        storeData,
        totals: {
          buyPct: storeData.reduce((sum: any, s: any) => sum + s.buyPct, 0),
          salesPct: storeData.reduce((sum: any, s: any) => sum + s.salesPct, 0),
          userBuyPct: storeData.reduce((sum: any, s: any) => sum + s.userBuyPct, 0),
          otbValue: storeData.reduce((sum: any, s: any) => sum + s.otbValue, 0)
        }
      };
    });
  ...
  const renderCategoryTab = () => {
    ...
    const calculateGenderTotals = (genderGroup: any) => {
      let totals: any = { buyPct: 0, salesPct: 0, stPct: 0, buyProposed: 0, otbProposed: 0, varPct: 0, otbSubmitted: 0, buyActual: 0 };
      genderGroup.categories.forEach((cat: any) => {
        cat.subCategories.forEach((subCat: any) => {
          const key = `${genderGroup.gender.id}_${cat.id}_${subCat.id}`;
    ...
    const filteredData = CATEGORY_STRUCTURE.filter((genderGroup: any) => {
              ...
              >
                {selectedVersion === 'draft' ? (
                  ...
                  <>
                    ...
                    <span>Version {versions.find((v: any) => v.id === selectedVersion)?.versionNumber}</span>
                  </>
                )}
