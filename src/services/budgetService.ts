...
export const budgetService = {
  ...
  async getAll(filters: any = {}) {
    const response: any = await api.get('/budgets', { params: filters });
  ...
  async getStatistics() {
    const response: any = await api.get('/budgets/statistics');
    return response.data.data || response.data;
  },
  ...
  async submit(id: any) {
    const response: any = await api.post(`/budgets/${id}/submit`);
    return response.data.data || response.data;
  },

  // Approve Level 1
  async approveL1(id: any, comment: string = '') {
    const response: any = await api.post(`/budgets/${id}/approve/level1`, {
      action: 'APPROVED',
      comment
    });
    return response.data.data || response.data;
  ...
  async approveL2(id: any, comment: string = '') {
    const response: any = await api.post(`/budgets/${id}/approve/level2`, {
      action: 'APPROVED',
      comment
    });
    return response.data.data || response.data;
  ...
  async delete(id: any) {
    const response: any = await api.delete(`/budgets/${id}`);
    return response.data;
  }
};
